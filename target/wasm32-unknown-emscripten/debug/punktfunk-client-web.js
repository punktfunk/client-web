...
function findWasmBinary() {
  if (Module['locateFile']) {
    return locateFile('punktfunk_client_web.wasm');
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('punktfunk_client_web.wasm', import.meta.url).href;
}
...
// Begin JS library code
  ...
  function _pf_gl_setup() {
          const mod = Module;
          if (mod.__pfUiCtx) {
              GL.makeContextCurrent(mod.__pfUiCtx);
              return 1;
          }
          const canvas = document.getElementById("pf-ui");
          if (!canvas)
              return 0;
  ...
  function _pf_video_au(ptr, len, ptsUs, key) {
          if (!Module.__pfOnAccessUnit)
              return;
  ...
  function _pf_video_config(codec, width, height) {
          if (Module.__pfOnVideoConfig)
              Module.__pfOnVideoConfig(codec, width, height);
      }
  ...
  function _pf_wt_ctl_send(ptr, len) {
          if (!pfNet.ctl)
              return;
  ...
  function _pf_wt_connect(urlPtr, hashPtr) {
          const url = UTF8ToString(urlPtr);
          const hex = UTF8ToString(hashPtr);
          try {
              const opts = { allowPooling: false };
  ...
  function _pf_wt_ctl_open() {
          if (!pfNet.wt || pfNet.ctl)
              return 0;
          pfNet.wt.createBidirectionalStream().then(function (stream) {
              ...
              if (Module.__pfOnCtlReady)
                  Module.__pfOnCtlReady();
          }, function (e) {
  ...
  load:function () {
              if (pfCred.key)
                  return Promise.resolve(pfCred.key);
  ...
  function _pf_device_init(hashPtr) {
          const hex = UTF8ToString(hashPtr);
          pfCred
              .load()
              .then(function (pair) {
              return crypto.subtle.exportKey("spki", pair.publicKey).then(function (spki) {
                  ...
                  if (Module.__pfOnDeviceReady)
                      Module.__pfOnDeviceReady();
              });
          })
              .catch(function (e) {
...
unexportedSymbols.forEach(unexportedRuntimeSymbol);
  ...
  Module['_pf_wt_connect'] = _pf_wt_connect;
  Module['_pf_wt_close'] = _pf_wt_close;
  Module['_pf_wt_ctl_open'] = _pf_wt_ctl_open;
  Module['_pf_device_init'] = _pf_device_init;
  Module['_pf_device_sign'] = _pf_device_sign;
...
var wasmImports = {
  ...
  /** @export */
  pf_gl_setup: _pf_gl_setup,
  /** @export */
  pf_video_au: _pf_video_au,
  /** @export */
  pf_video_config: _pf_video_config,
  /** @export */
  pf_wt_ctl_send: _pf_wt_ctl_send,
  /** @export */
  pf_wt_send: _pf_wt_send,
  /** @export */
...
var _pf_cred_signed = Module['_pf_cred_signed'] = createExportWrapper('pf_cred_signed', 2);
var _pf_ctl_recv = Module['_pf_ctl_recv'] = createExportWrapper('pf_ctl_recv', 2);
var _pf_pair_begin = Module['_pf_pair_begin'] = createExportWrapper('pf_pair_begin', 4);
var _pf_session_frames = Module['_pf_session_frames'] = createExportWrapper('pf_session_frames', 0);
var _pf_session_hello = Module['_pf_session_hello'] = createExportWrapper('pf_session_hello', 4);
var _pf_session_phase = Module['_pf_session_phase'] = createExportWrapper('pf_session_phase', 0);
var _pf_session_pump = Module['_pf_session_pump'] = createExportWrapper('pf_session_pump', 0);
var _pf_cred_phase = Module['_pf_cred_phase'] = createExportWrapper('pf_cred_phase', 0);
var _pf_cred_sign_len = Module['_pf_cred_sign_len'] = createExportWrapper('pf_cred_sign_len', 0);
var _pf_cred_sign_ptr = Module['_pf_cred_sign_ptr'] = createExportWrapper('pf_cred_sign_ptr', 0);
var _pf_device_fingerprint_hex = Module['_pf_device_fingerprint_hex'] = createExportWrapper('pf_device_fingerprint_hex', 1);
var _pf_device_set = Module['_pf_device_set'] = createExportWrapper('pf_device_set', 3);
var _main = Module['_main'] = createExportWrapper('main', 2);
var _pf_net_blast = Module['_pf_net_blast'] = createExportWrapper('pf_net_blast', 2);
var _pf_net_drain = Module['_pf_net_drain'] = createExportWrapper('pf_net_drain', 0);
var _pf_rx_base = Module['_pf_rx_base'] = createExportWrapper('pf_rx_base', 0);
var _pf_rx_claim = Module['_pf_rx_claim'] = createExportWrapper('pf_rx_claim', 0);
var _pf_rx_commit = Module['_pf_rx_commit'] = createExportWrapper('pf_rx_commit', 2);
var _pf_rx_dropped = Module['_pf_rx_dropped'] = createExportWrapper('pf_rx_dropped', 0);
var _pf_rx_stride = Module['_pf_rx_stride'] = createExportWrapper('pf_rx_stride', 0);
var _pf_frame = Module['_pf_frame'] = createExportWrapper('pf_frame', 2);
var _pf_key = Module['_pf_key'] = createExportWrapper('pf_key', 3);
var _pf_start = Module['_pf_start'] = createExportWrapper('pf_start', 2);
