import { createComponent, createRenderEffect, mergeProps, untrack } from "./solid-js.js";
import { Aliases, ChildProperties, DelegatedEvents, Properties, SVGElements, SVGNamespace, addEventListener, classList, delegateEvents, dynamicProperty, getPropAlias, insert, setAttribute, setAttributeNS, spread, style } from "./solid-js_web.js";
...
function parse(html) {
	...
	html.replace(tagRE, (tag, index) => {
		...
		if (isOpen && !isComment) {
			...
			if (!current.voidElement && nextChar && nextChar !== "<") pushTextNode(current.children, html, start);
		...
		if (isComment) {
			if (level < 0) pushCommentNode(result, tag);
...
function createHTML(r, { delegateEvents = true, functionBuilder = (...args) => new Function(...args) } = {}) {
	...
	function createTemplate(statics, opt) {
		...
		const [html, code] = parseTemplate(parse(markup), opt.funcBuilder), templates = [];
	...
	function parseAttribute(node, tag, name, value, isSVG, isCE, options) {
		if (name.slice(0, 2) === "on") {
			...
			const childOptions = Object.assign({}, options, { exprs: [] }), count = options.counter;
	...
	function processChildren(node, options) {
		...
		if (node.children.length > 1) for (let i = 0; i < node.children.length; i++) {
	...
	function parseNode(node, options) {
		if (node.type === "fragment") {
			...
			options.decl.push(`${tag} = ${options.path}.${options.first ? "firstChild" : "nextSibling"}`);
			if (node.content === "#") {
				if (options.multi) options.exprs.push(`r.insert(${options.parent}, exprs[${options.counter++}], ${tag})`);
	...
	function parseTemplate(nodes, funcBuilder) {
		...
		return [[origNodes].concat(options.templateNodes).map((t) => stringify(t)), funcBuilder("tmpls", "exprs", "r", options.decl.join(",\n") + ";\n" + options.exprs.join(";\n") + (toplevel ? "" : `;\nreturn _$el${id};\n`))];
	}
